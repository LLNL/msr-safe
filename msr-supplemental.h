...
/* Notes (References are to the September 2013 edition of the Intel documentation.)
 *
 * TIME_STAMP_COUNTER	64-bit writable counter, but we enforce RO here.
 *			See section 17.3.
 *
 */ 
...
/* 													*/ \
/* Section 17.3.	Architectural as of 05_01H */ \
/* Writable counter; read-only per policy.	           					*/ \
SMSR_ENTRY( SMSR_TIME_STAMP_COUNTER,	{0x010,	SMSR_READALL, SMSR_READALL, SMSR_RO,       SMSR_RO      }),\
/* 													*/ \
/* Read only; only bits 50-52 are used.									*/ \
/* Note section 18.14.5 lists bits 8-12 for bus ration if XE mode is disabled.  Ignore for now.		*/ \
SMSR_ENTRY( SMSR_PLATFORM_ID,		{0x017,	SMSR_READALL, SMSR_READALL, SMSR_RO,       SMSR_RO      }),\
/* 													*/ \
