...
static struct whitelist_entry *find_in_whitelist(__u8 bus,
                                                 __u8 device,
                                                 __u8 function,
                                                 __u64 csr);
...
static int parse_next_whitelist_entry(char *inbuf,
                                      char **nextinbuf,
                                      struct whitelist_entry *entry);

static ssize_t read_whitelist(struct file *filp,
                              char __user *buf,
                              size_t count,
                              loff_t *ppos);
 ...
 */
static ssize_t write_whitelist(struct file *filp, const char __user *buf, size_t count, loff_t *ppos)
{
    int err = 0;
    ...
    for (entry = whitelist, s = kbuf, res = 1; res > 0; entry++)
    {
        ...
        if (res)
        {
            ...
            {
                inheritance = find_in_whitelist((entry->csr >> 56) & 0xFF, (entry->csr >> 48) & 0xFF, (entry->csr >> 44) & 0x0F, (entry->csr >> 32) & 0xFFF);
                if (inheritance)
                {
            ...
            if ((entry->csr & 0x00000000FFFFF000) != prev_csr)
            {
                ...
                if (entry->csr & (0x1 << 24))
                {
                    entry->maps0 = __pcie_ioremap(s0_uncore_bus, (entry->csr >> 16) & 0xFF, (entry->csr >> 12) & 0x0F);
                    lasts0 = temp[itr] = entry->maps0;
                    entry->maps1 = __pcie_ioremap(s1_uncore_bus, (entry->csr >> 16) & 0xFF, (entry->csr >> 12) & 0x0F);
                    lasts1 = temp[itr + 1] = entry->maps1;
                }
                else
                {
                    entry->maps0 = __pcie_ioremap(s0_core_bus, (entry->csr >> 16) & 0xFF, (entry->csr >> 12) & 0x0F);
                    lasts0 = temp[itr] = entry->maps0;
                    entry->maps1 = __pcie_ioremap(s1_core_bus, (entry->csr >> 16) & 0xFF, (entry->csr >> 12) & 0x0F);
                    lasts1 = temp[itr + 1] = entry->maps1;
                }
                itr += 2;
            }
...
static ssize_t read_whitelist(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
    loff_t idx = *ppos;
    ...
    *ppos = 0;
    ...
    len = sprintf(kbuf, "CSR: %016llx Write Mask: %016llx Map: %p | %p\n", e.csr, e.wmask, e.maps0, e.maps1);
...
int csr_whitelist_init(void)
{
    ...
    dev = device_create(cdev_class, NULL, MKDEV(majordev, 0), NULL, "csr_whitelist");
    if (IS_ERR(dev))
    {
        err = PTR_ERR(dev);
        csr_whitelist_cleanup();
